...
test('defaultAdapter returns a complete adapter implementation', t => {
  verifyDefaultAdapter(MDLSimpleMenuFoundation, [
    ...
    'notifySelected', 'saveFocus', 'restoreFocus', 'isMenuFocused', 'focusMenu', 'getFocusedItemIndex',
    'focusItemAtIndex'
  ], t);
  t.end();
});
...
testFoundation('#open focuses the menu at the end of the animation', t => {
  ...
  td.when(mockNow()).thenReturn(500);
  mockRaf.flush();
  t.doesNotThrow(() => td.verify(mockAdapter.focusMenu()));
...
test('on escape keyup closes the menu', t => {
  const {foundation, mockAdapter} = setupTest();
  const handlers = captureHandlers(mockAdapter, 'registerInteractionHandler');
  const clock = lolex.install();
  const raf = createMockRaf();
