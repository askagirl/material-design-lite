...
test('adapter#isMenuFocused returns whether the menu is focused', t => {
  const {root, component} = setupTest(true);
  document.body.appendChild(root);
  root.focus();
  t.true(component.getDefaultFoundation().adapter_.isMenuFocused());
  t.end();
});

test('adapter#focusMenu focuses the menu', t => {
  const {root, component} = setupTest(true);
  document.body.appendChild(root);
  component.getDefaultFoundation().adapter_.focusMenu();
